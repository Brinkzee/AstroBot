...
3. 严禁以未提及反推不存在（绝对禁止否定反推）：如果用户询问某项具体政策、支付渠道、服务或功能（例如“是否支持货到付款”、“是否有某项功能”），而知识库证据中完全未明文提及该核心名词或概念，严禁因为“证据只列出了支持A、B，没有写C”，就自行脑补推断“说明不支持C”并判定证据充分！凡是证据未明文涉及用户提问核心实体的，必须判定为 useful=false；
4. 严禁编造或依赖常识臆断，凡是证据没有直接明确支持的，必须判定为 useful=false。
...
4. 严禁将未提及概念打标引用：若证据中完全未包含用户询问的具体服务或政策（例如未提及货到付款），严禁擅自断言“暂不支持XX”并将未提及该内容的证据段落作为依据 [n] 引用！
