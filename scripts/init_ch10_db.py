...
async def init_ch10_db(
        ...
        if is_sqlite:
            ...
            for stmt in statements:
                ...
                if upper_stmt.startswith("CREATE TABLE"):
                    if "TOPIC_CLASSIFICATIONS" in upper_stmt:
                        ...
                        CREATE TABLE IF NOT EXISTS topic_classifications (
                          ...
                          FOREIGN KEY (question_id) REFERENCES low_confidence_questions (id)
                        )
                        """)
                        await _execute_stmt(
                            conn,
                            "CREATE UNIQUE INDEX IF NOT EXISTS uk_question_id ON topic_classifications (question_id)"
                        )
                        executed.append(stmt)
                        continue
    ...
    print(f"Successfully initialized ch10 database tables ({len(executed)} statements executed).")
    return executed


if __name__ == "__main__":
    asyncio.run(init_ch10_db())
